...
data "http" "exec" {
  ...
  lifecycle {
    ...
    postcondition {
      condition     = google_cloud_run_v2_job.job.generation == lookup(lookup(lookup(jsondecode(self.response_body), "metadata"), "labels"), "run.googleapis.com/jobGeneration")
      error_message = "Executed wrong generation of job"
    }
  }
}
